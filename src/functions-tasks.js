...
function getPolynom(...args) {
  return args.length === 0
    ? null
    : (inputVal) =>
        args.reduce((acc, coeff, index) => {
          return acc + coeff * inputVal ** (args.length - index - 1);
        }, 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const parametersString = JSON.stringify(args).slice(1, -1);
    logFunc(`${func.name}(${parametersString}) starts`);
    const output = func(...args);
    logFunc(`${func.name}(${parametersString}) ends`);
    return output;
  };
}
